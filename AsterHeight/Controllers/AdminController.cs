    ...
    public class AdminController : AdminBaseController
    {
        public ActionResult AdminDashBoard()
        {
            ...
            try
            {
                ...
                ViewBag.TotalApproveLuckyDraw = Ds.Tables[0].Rows[0]["TotalApproveLuckyDraw"].ToString();
                ViewBag.TotalPendingLuckyDraw = Ds.Tables[0].Rows[0]["TotalPendingLuckyDraw"].ToString();
                ViewBag.TotalBusiness = Ds.Tables[0].Rows[0]["TotalBusiness"].ToString();
                ViewBag.TotalApprovedBusiness = Ds.Tables[0].Rows[0]["TotalApprovedBusiness"].ToString();
        ...
        public ActionResult RegistrationAction(string CustomerID, string SponsorId, string FirstName, string LastName, string Email, string MobileNo, string PanCard, string Address, string Gender, string OTP, string Pincode, string Leg, string PaymentMode, string Amount, string BookingNo, string TransactionDate, string TransactionNo, string BankName, string BranchName, string AdjustmentId, string PaymentDate)
        {
            Home obj = new Home();
            ...
            try
            {
                ...
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    if (ds.Tables[0].Rows[0]["Msg"].ToString() == "1")
                    {
                        ...
                        try
                        {
                            string str2 = "Dear " + ds.Tables[0].Rows[0]["Name"].ToString() + ",You have been successfully registered as Associate in Blossom City. Your Login Id is " + ds.Tables[0].Rows[0]["LoginId"].ToString() + " and password is " + Crypto.Decrypt(ds.Tables[0].Rows[0]["Password"].ToString());
                            BLSMS.SendSMSNew(MobileNo, str2);
                        }
                        catch (Exception ex)
                        {
        ...
        public ActionResult EditAssociateDetail(string UserID)
        {
            Home obj = new Home();
            if (UserID != null)
            {
                ...
                if (dsPlotDetails != null && dsPlotDetails.Tables.Count > 0)
                {
                    ...
                    // objViewBag.Disabled = "disabled";
                    obj.AdharNumber = dsPlotDetails.Tables[0].Rows[0]["AdharNumber"].ToString();
                    obj.AccountNumber = dsPlotDetails.Tables[0].Rows[0]["MemberAccNo"].ToString();
                    obj.AccountHolderName = dsPlotDetails.Tables[0].Rows[0]["BankHolderName"].ToString();
                    obj.BankName = dsPlotDetails.Tables[0].Rows[0]["MemberBankName"].ToString();
                    obj.BankBranch = dsPlotDetails.Tables[0].Rows[0]["MemberBranch"].ToString();
                    obj.IFSC = dsPlotDetails.Tables[0].Rows[0]["IFSCCode"].ToString();
                    obj.Signature = dsPlotDetails.Tables[0].Rows[0]["Signature"].ToString();
                }
            }
            else
        ...
        [HttpPost]
        [ActionName("EditAssociateDetail")]
        ...
        [HttpPost]
        public ActionResult TopUp(Customer model)
        {
            try
            {
                model.TransactionDate = string.IsNullOrEmpty(model.TransactionDate) ? null : Common.ConvertToSystemDate(model.TransactionDate, "dd/MM/yyyy");
                ...
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    if (ds.Tables[0].Rows[0]["Msg"].ToString() == "1")
                    {
                        TempData["TopUp"] = "Top-Up Saved successfully.";
                    }
                    else
                    {
        ...
        public ActionResult LoadData()
        {
            ...
            //Find Order Column
            var searchParam = Request.Form.GetValues("search[value]").FirstOrDefault();
            ...
            int recordsTotal = 0;
            Plot model = new Plot();
            List<Testing> lst = new List<Testing>();
            model.page = skip;
            model.pageSize = pageSize;
            model.sortColumn = sortColumn;
            model.sortColumnDir = sortColumnDir;
            DataSet ds = model.ServerSideDemo();
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow r in ds.Tables[0].Rows)
                {
                ...
                recordsTotal = Convert.ToInt32(ds.Tables[1].Rows[0][0]);
            }
        ...
        public ActionResult GetActualLeftRightBusiness()
        {
            //Reports model = new Reports();
            //List<Reports> lst = new List<Reports>();
            //DataSet ds = model.GetActualLeftRightBusinessList();
            //if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            //{
            //    foreach (DataRow dr in ds.Tables[0].Rows)
            //    {
            //        Reports obj = new Reports();
            //        obj.UserId = dr["PK_UserId"].ToString();
            //        obj.LoginId = dr["LoginId"].ToString();
            //          obj.Name = dr["Name"].ToString();
            //        obj.JoiningDate = dr["JoiningDate"].ToString();
            //        obj.LeftBusiness = dr["LeftBusiness"].ToString();
            //        obj.RightBusiness = dr["RightBusiness"].ToString();
            //        lst.Add(obj);
            //    }
            //    model.ActualLeftRightBusinessList = lst;
            //}
            return View();
        }

        [HttpPost]
        [ActionName("GetActualLeftRightBusiness")]
        [OnAction(ButtonName = "Search")]
        public ActionResult GetActualLeftRightBusiness(Reports model)
        {
            List<Reports> lst = new List<Reports>();
